...
func PrintStructField(s interface{}) {
	...
	fmt.Printf("%#v", field0) //reflect.StructField{Name:"Name", PkgPath:"", Type:(*reflect.rtype)(0x10b3b00), Tag:"json:\"name\"", Offset:0x0, Index:[]int{0}, Anonymous:false}
	fmt.Println()
...
func PrintStructFn(s interface{}) {
	...
	// v.Method(1).Call(nil) //这里"Method"传入的值需要猜测,不准确
	v.MethodByName("Print").Call(nil)

	info1 := v.MethodByName("GetInfo").Call(nil)
	fmt.Println(info1)
	fmt.Println("--------------------------------")
	//4.执行方法传入参数(注意需要使用<<值变量>>,并且注意参数,接收的参数是[]reflect.Value的切片)
	var parmas []reflect.Value
	parmas = append(parmas, reflect.ValueOf("李四"))
	parmas = append(parmas, reflect.ValueOf(23))
	parmas = append(parmas, reflect.ValueOf(99))
	v.MethodByName("SetInfo").Call(parmas) //执行方法传入参数
